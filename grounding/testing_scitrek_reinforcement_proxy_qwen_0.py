...
if __name__ == "__main__":
    ...
    model_name_official = "checkpoint_scitrek_reinforcement_qwen_0/global_step500_hf"
    model_name_save = "qwen3_4b_instruct_0527_reinforcement_0_500"
    target_mode = "proxy"
    ...
    for sample_index, sample in tqdm(enumerate(samples_test), total=len(samples_test),
        ...
        if "generations" not in sample.keys():
            ...
            if len(working_instances) == inference_batch_size or (len(working_instances) > 0 and sample_index + 1 == len(samples_test)):
                inputs = []
