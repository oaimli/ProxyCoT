...
def get_context(article_ids, metadata, sentences_all):
    ...
    random_sentences = random.sample(sentences, len(metadata_sentences))
...
if __name__ == "__main__":
    ...
    model_name_official = "checkpoint_scitrek_random/_actor/global_step20_hf"
    model_name_save = "qwen3_4b_instruct_0527_random"
    target_mode = "proxy"
    ...
    with open("sentences_all_scitrek.json") as f:
        ...
        if "generations" not in sample.keys():
            ...
            if len(working_instances) == inference_batch_size or (len(working_instances) > 0 and sample_index == len(samples_test) - 1):
                ...
                for working_instance in working_instances:
                    ...
                    metadata = get_proxy(working_sample, articles_all)
                    article_ids = working_sample["articles"]
                    random_sentences = get_context(article_ids, metadata, sentences_all)
