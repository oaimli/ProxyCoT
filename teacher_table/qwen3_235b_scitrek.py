...
sys.path.append("../")
from preparation.scitrek.data_loading import load_train, load_test, load_articles, get_tables
...
if __name__ == "__main__":
    ...
    instruction = open("instruction_table_scitrek.txt").read()
        ...
        if "generations" not in sample.keys():
            ...
            if len(working_instances) == inference_batch_size or (len(working_instances) > 0 and sample_index == len(samples_dev) - 1):
                ...
                for working_instance in working_instances:
                    ...
                    table_articles, table_article_author, table_citing_cited = get_tables(working_sample, articles_all)
                    ...
                    prompt_content = prompt_content.replace("<table-articles>", table_articles_text)
                    prompt_content = prompt_content.replace("<table-article_author>", table_article_author_text)
                    prompt_content = prompt_content.replace("<table-citing_cited>", table_citing_cited_text)
