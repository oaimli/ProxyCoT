...
def _extract_response_text(response):
    # Responses API has output_text; fall back to output content if needed
...
def get_api_results(prompt_input, config):
    ...
    # OpenAI Responses API (for openai-managed models, including gpt-4/5 families)
    if config['type'] == 'openai':
        client = OpenAI(api_key=config['args']['api_key'],
                        base_url=config['args']['api_url'] if config['args']['api_url'] != '' else None)
        try:
            # Some models (e.g., gpt-5-* reasoning) do not support temperature.
            # Pass temperature only if explicitly allowed; otherwise omit.
            run_args = config.get('run_args', {}) if isinstance(config, dict) else {}
            allow_temperature = run_args.get('allow_temperature', False)
            kwargs = {
                "model": config['args']['api_name'],
                "input": [{"role": "user", "content": prompt}],
            }
            if allow_temperature and 'temperature' in run_args:
                kwargs["temperature"] = run_args["temperature"]

            response = client.responses.create(**kwargs)
            text = _extract_response_text(response)
            text = text if text is not None else ""
            if os.getenv("VERBOSE_EVAL") == "1":
    ...
    # vLLM OpenAI-compatible server (chat completions)
    if config['type'] == 'vllm':
        client = OpenAI(api_key=config['args']['api_key'],
                        base_url=config['args']['api_url'] if config['args']['api_url'] != '' else None)
        try:
